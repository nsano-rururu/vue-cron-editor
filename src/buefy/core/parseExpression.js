...
function parseDayOfWeek(expr) {
    ...
    let groups = expr.match(/([a-zA-Z0-9]+)(,[a-zA-Z0-9]+)?(,[a-zA-Z0-9]+)?(,[a-zA-Z0-9]+)?(,[a-zA-Z0-9]+)?(,[a-zA-Z0-9]+)?(,[a-zA-Z0-9]+)?/);
    if (groups == null)
...
const isAnyTime = (token) => token.type == "asterisk" || (token.type == "number" && token.value == 0);
export const parseExpression = (expression) => {
    ...
    if (cron.minutes.type == "cronNumber" &&
