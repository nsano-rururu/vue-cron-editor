...
const isResultValid = (
    ...
    const isValid = cronValidator.isValidCron(expr, options);
...
test("with basic syntax", () => {
    fc.assert(
        fc.property(state, (e) => {
            const state = e as UiState;

            const expr = cronExpressions.buildExpression("basic", state);
...
test("when aliasing week days", () => {
    fc.assert(
        fc.property(state, (e) => {
            const state = e as UiState;
            const expr = cronExpressions.buildExpression("quartz", state);
