...
const isResultValid = (state, expr, options) => {
    let isValid = cronValidator.isValidCron(expr, options);
    if (!isValid) {
        console.log("Invalid expression produced: " +
            expr +
            " for state " +
    ...
    try {
        ...
        if (parsed.type != state.type) {
            console.log(`expected to parse ${JSON.stringify(state)} -> ${expr} as ${state.type} but was ${parsed.type}`);
            return false;
        }
    }
    catch (e) {
        console.log("Cannot parse produced expression: " +
            expr +
            " for state " +
            JSON.stringify(state));
        throw e;
...
//# sourceMappingURL=buildsOnlyValidExpressions.spec.js.map